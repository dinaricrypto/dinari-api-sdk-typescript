...
export const parseClientAuthHeaders = (req: IncomingMessage, required?: boolean): Partial<ClientOptions> => { const apiKeyID = Array.isArray(req.headers['x-api-key-id']) ? req.headers['x-api-key-id'][0] : req.headers['x-api-key-id']
const apiSecretKey = Array.isArray(req.headers['x-api-secret-key']) ? req.headers['x-api-secret-key'][0] : req.headers['x-api-secret-key']
