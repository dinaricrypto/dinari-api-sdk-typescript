...
export { ClientOptions } from '@dinari/api-sdk';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/dinari',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the dinari MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Dinari({
    ...{ environment: (readEnv('DINARI_ENVIRONMENT') || undefined) as any },
    logger,
