...
const remoteStainlessHandler = async (
  ...
  DINARI_API_KEY_ID: requireValue(readEnv('DINARI_API_KEY_ID') ?? client.apiKeyID, 'set DINARI_API_KEY_ID environment variable or provide apiKeyID client option'),
  DINARI_API_SECRET_KEY: requireValue(readEnv('DINARI_API_SECRET_KEY') ?? client.apiSecretKey, 'set DINARI_API_SECRET_KEY environment variable or provide apiSecretKey client option'),
  DINARI_BASE_URL: readEnv('DINARI_BASE_URL') ?? readEnv('DINARI_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
};
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { environment: (readEnv('DINARI_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@dinari/api-sdk';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
          ...
          ...(client.apiKeyID != null ? { apiKeyID: client.apiKeyID } : undefined),
          ...(client.apiSecretKey != null ? { apiSecretKey: client.apiSecretKey } : undefined),
          defaultHeaders: {
