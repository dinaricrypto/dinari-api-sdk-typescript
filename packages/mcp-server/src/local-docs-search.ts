...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; is_fractionable: boolean; is_tradable: boolean; name: string; symbol: string; tokens: string[]; cik?: string; composite_figi?: string; cusip?: string; description?: string; display_name?: string; logo_url?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedStockResponse:v1'; }",
    markdown:
      "## list\n\n`client.v2.marketData.stocks.list(limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string, symbols?: string[]): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedStockResponse:v1'; }`\n\n**get** `/api/v2/market_data/stocks/`\n\nGet a list of `Stocks`.\n\n### Parameters\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n- `symbols?: string[]`\n  List of `Stock` symbols to query. If not provided, all `Stocks` are returned.\n\n### Returns\n\n- `{ data: { id: string; is_fractionable: boolean; is_tradable: boolean; name: string; symbol: string; tokens: string[]; cik?: string; composite_figi?: string; cusip?: string; description?: string; display_name?: string; logo_url?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedStockResponse:v1'; }`\n\n  - `data: { id: string; is_fractionable: boolean; is_tradable: boolean; name: string; symbol: string; tokens: string[]; cik?: string; composite_figi?: string; cusip?: string; description?: string; display_name?: string; logo_url?: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedStockResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst stocks = await client.v2.marketData.stocks.list();\n\nconsole.log(stocks);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    params: ['limit?: number;', 'next?: string;', "order?: 'asc' | 'desc';", 'previous?: string;'],
    response:
      "{ data: { id: string; ex_date: string; payable_date: string; record_date: string; split_from: number; split_to: number; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE'; stock_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedStockSplitResponse:v1'; }",
    markdown:
      "## list\n\n`client.v2.marketData.stocks.splits.list(limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: stock_split[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedStockSplitResponse:v1'; }`\n\n**get** `/api/v2/market_data/stocks/splits`\n\n\nGet a list of stock splits for `Stocks` available for trade via Dinari.\nThe splits are ordered by the date they were created, with the most recent split first.\n\nIn an example 10-for-1 stock split, trading will be halted for the stock at the end of the `payable_date`, as\nthe split transitions from `PENDING` to `IN_PROGRESS`. This usually occurs over a weekend to minimize trading\ndisruptions. Each share of stock owned by a shareholder will then be converted into 10 shares, and the split\nbecomes `COMPLETE` as trading resumes on the `ex_date` with new split-adjusted prices.\n\n\n### Parameters\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; ex_date: string; payable_date: string; record_date: string; split_from: number; split_to: number; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE'; stock_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedStockSplitResponse:v1'; }`\n\n  - `data: { id: string; ex_date: string; payable_date: string; record_date: string; split_from: number; split_to: number; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE'; stock_id: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedStockSplitResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst paginatedStockSplit = await client.v2.marketData.stocks.splits.list();\n\nconsole.log(paginatedStockSplit);\n```",
    perLanguage: {
      typescript: {
        method: 'client.v2.marketData.stocks.splits.list',
        example:
          "import Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari({\n  apiKeyID: process.env['DINARI_API_KEY_ID'], // This is the default and can be omitted\n  apiSecretKey: process.env['DINARI_API_SECRET_KEY'], // This is the default and can be omitted\n});\n\nconst paginatedStockSplit = await client.v2.marketData.stocks.splits.list();\n\nconsole.log(paginatedStockSplit.data);",
      },
      python: {
        method: 'v2.market_data.stocks.splits.list',
        example:
          'import os\nfrom dinari_api_sdk import Dinari\n\nclient = Dinari(\n    api_key_id=os.environ.get("DINARI_API_KEY_ID"),  # This is the default and can be omitted\n    api_secret_key=os.environ.get("DINARI_API_SECRET_KEY"),  # This is the default and can be omitted\n)\npaginated_stock_split = client.v2.market_data.stocks.splits.list()\nprint(paginated_stock_split.data)',
      },
      java: {
        method: 'v2().marketData().stocks().splits().list',
        example:
          'package com.dinari.api.example;\n\nimport com.dinari.api.client.DinariClient;\nimport com.dinari.api.client.okhttp.DinariOkHttpClient;\nimport com.dinari.api.models.v2.marketdata.stocks.splits.PaginatedStockSplit;\nimport com.dinari.api.models.v2.marketdata.stocks.splits.SplitListParams;\n\npublic final class Main {\n    private Main() {}\n\n    public static void main(String[] args) {\n        DinariClient client = DinariOkHttpClient.fromEnv();\n\n        PaginatedStockSplit paginatedStockSplit = client.v2().marketData().stocks().splits().list();\n    }\n}',
      },
      go: {
        method: 'client.V2.MarketData.Stocks.Splits.List',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/dinaricrypto/dinari-api-sdk-go"\n\t"github.com/dinaricrypto/dinari-api-sdk-go/option"\n)\n\nfunc main() {\n\tclient := dinariapisdkgo.NewClient(\n\t\toption.WithAPIKeyID("My API Key ID"),\n\t\toption.WithAPISecretKey("My API Secret Key"),\n\t)\n\tpaginatedStockSplit, err := client.V2.MarketData.Stocks.Splits.List(context.TODO(), dinariapisdkgo.V2MarketDataStockSplitListParams{})\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", paginatedStockSplit.Data)\n}\n',
      },
      php: {
        method: 'v2->marketData->stocks->splits->list',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(\n  apiKeyID: 'My API Key ID',\n  apiSecretKey: 'My API Secret Key',\n  environment: 'sandbox',\n);\n\n$paginatedStockSplit = $client->v2->marketData->stocks->splits->list(\n  limit: 20, next: 'next', order: 'asc', previous: 'previous'\n);\n\nvar_dump($paginatedStockSplit);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; ex_date: string; payable_date: string; record_date: string; split_from: number; split_to: number; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE'; stock_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedStockSplitResponse:v1'; }",
    markdown:
      "## list_for_stock\n\n`client.v2.marketData.stocks.splits.listForStock(stock_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: stock_split[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedStockSplitResponse:v1'; }`\n\n**get** `/api/v2/market_data/stocks/{stock_id}/splits`\n\n\nGet a list of stock splits for a specific `Stock`.\nThe splits are ordered by the date they were created, with the most recent split first.\n\nIn an example 10-for-1 stock split, trading will be halted for the stock at the end of the `payable_date`, as\nthe split transitions from `PENDING` to `IN_PROGRESS`. This usually occurs over a weekend to minimize trading\ndisruptions. Each share of stock owned by a shareholder will then be converted into 10 shares, and the split\nbecomes `COMPLETE` as trading resumes on the `ex_date` with new split-adjusted prices.\n\n\n### Parameters\n\n- `stock_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; ex_date: string; payable_date: string; record_date: string; split_from: number; split_to: number; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE'; stock_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedStockSplitResponse:v1'; }`\n\n  - `data: { id: string; ex_date: string; payable_date: string; record_date: string; split_from: number; split_to: number; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE'; stock_id: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedStockSplitResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst paginatedStockSplit = await client.v2.marketData.stocks.splits.listForStock('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(paginatedStockSplit);\n```",
    perLanguage: {
      typescript: {
        method: 'client.v2.marketData.stocks.splits.listForStock',
        example:
          "import Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari({\n  apiKeyID: process.env['DINARI_API_KEY_ID'], // This is the default and can be omitted\n  apiSecretKey: process.env['DINARI_API_SECRET_KEY'], // This is the default and can be omitted\n});\n\nconst paginatedStockSplit = await client.v2.marketData.stocks.splits.listForStock(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n);\n\nconsole.log(paginatedStockSplit.data);",
      },
      python: {
        method: 'v2.market_data.stocks.splits.list_for_stock',
        example:
          'import os\nfrom dinari_api_sdk import Dinari\n\nclient = Dinari(\n    api_key_id=os.environ.get("DINARI_API_KEY_ID"),  # This is the default and can be omitted\n    api_secret_key=os.environ.get("DINARI_API_SECRET_KEY"),  # This is the default and can be omitted\n)\npaginated_stock_split = client.v2.market_data.stocks.splits.list_for_stock(\n    stock_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n)\nprint(paginated_stock_split.data)',
      },
      java: {
        method: 'v2().marketData().stocks().splits().listForStock',
        example:
          'package com.dinari.api.example;\n\nimport com.dinari.api.client.DinariClient;\nimport com.dinari.api.client.okhttp.DinariOkHttpClient;\nimport com.dinari.api.models.v2.marketdata.stocks.splits.PaginatedStockSplit;\nimport com.dinari.api.models.v2.marketdata.stocks.splits.SplitListForStockParams;\n\npublic final class Main {\n    private Main() {}\n\n    public static void main(String[] args) {\n        DinariClient client = DinariOkHttpClient.fromEnv();\n\n        PaginatedStockSplit paginatedStockSplit = client.v2().marketData().stocks().splits().listForStock("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e");\n    }\n}',
      },
      go: {
        method: 'client.V2.MarketData.Stocks.Splits.ListForStock',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/dinaricrypto/dinari-api-sdk-go"\n\t"github.com/dinaricrypto/dinari-api-sdk-go/option"\n)\n\nfunc main() {\n\tclient := dinariapisdkgo.NewClient(\n\t\toption.WithAPIKeyID("My API Key ID"),\n\t\toption.WithAPISecretKey("My API Secret Key"),\n\t)\n\tpaginatedStockSplit, err := client.V2.MarketData.Stocks.Splits.ListForStock(\n\t\tcontext.TODO(),\n\t\t"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n\t\tdinariapisdkgo.V2MarketDataStockSplitListForStockParams{},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", paginatedStockSplit.Data)\n}\n',
      },
      php: {
        method: 'v2->marketData->stocks->splits->listForStock',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(\n  apiKeyID: 'My API Key ID',\n  apiSecretKey: 'My API Secret Key',\n  environment: 'sandbox',\n);\n\n$paginatedStockSplit = $client->v2->marketData->stocks->splits->listForStock(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n  limit: 20,\n  next: 'next',\n  order: 'asc',\n  previous: 'previous',\n);\n\nvar_dump($paginatedStockSplit);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; entity_type: 'INDIVIDUAL' | 'ORGANIZATION'; is_kyc_complete: boolean; name?: string; nationality?: string; reference_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedEntityResponse:v1'; }",
    markdown:
      "## list\n\n`client.v2.entities.list(limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string, reference_id?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedEntityResponse:v1'; }`\n\n**get** `/api/v2/entities/`\n\nGet a list of direct `Entities` your organization manages. These `Entities` represent individual customers of your organization.\n\n### Parameters\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n- `reference_id?: string`\n  Case sensitive unique reference ID for the `Entity`.\n\n### Returns\n\n- `{ data: { id: string; entity_type: 'INDIVIDUAL' | 'ORGANIZATION'; is_kyc_complete: boolean; name?: string; nationality?: string; reference_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedEntityResponse:v1'; }`\n\n  - `data: { id: string; entity_type: 'INDIVIDUAL' | 'ORGANIZATION'; is_kyc_complete: boolean; name?: string; nationality?: string; reference_id?: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedEntityResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst entities = await client.v2.entities.list();\n\nconsole.log(entities);\n```",
    perLanguage: {
      typescript: {
      ...
      php: {
        method: 'v2->entities->list',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(\n  apiKeyID: 'My API Key ID',\n  apiSecretKey: 'My API Secret Key',\n  environment: 'sandbox',\n);\n\n$entities = $client->v2->entities->list(\n  limit: 20, next: 'next', order: 'asc', previous: 'previous', referenceID: 'x'\n);\n\nvar_dump($entities);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; created_dt: string; entity_id: string; is_active: boolean; jurisdiction: 'BASELINE' | 'US'; brokerage_account_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountResponse:v1'; }",
    markdown:
      "## list\n\n`client.v2.entities.accounts.list(entity_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedAccountResponse:v1'; }`\n\n**get** `/api/v2/entities/{entity_id}/accounts`\n\nGet a list of all `Accounts` that belong to a specific `Entity`. This `Entity` represents your organization itself, or an individual customer of your organization.\n\n### Parameters\n\n- `entity_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; created_dt: string; entity_id: string; is_active: boolean; jurisdiction: 'BASELINE' | 'US'; brokerage_account_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountResponse:v1'; }`\n\n  - `data: { id: string; created_dt: string; entity_id: string; is_active: boolean; jurisdiction: 'BASELINE' | 'US'; brokerage_account_id?: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedAccountResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst accounts = await client.v2.entities.accounts.list('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(accounts);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      go: {
        ...
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/dinaricrypto/dinari-api-sdk-go"\n\t"github.com/dinaricrypto/dinari-api-sdk-go/option"\n)\n\nfunc main() {\n\tclient := dinariapisdkgo.NewClient(\n\t\toption.WithAPIKeyID("My API Key ID"),\n\t\toption.WithAPISecretKey("My API Secret Key"),\n\t)\n\tresponse, err := client.V2.Entities.KYC.NewManagedCheck(\n\t\tcontext.TODO(),\n\t\t"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n\t\tdinariapisdkgo.V2EntityKYCNewManagedCheckParams{},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", response.EmbedURL)\n}\n',
      },
      php: {
        method: 'v2->entities->kyc->createManagedCheck',
        example:
  ...
  },
  {
    ...
    response:
      "{ data: { amount: number; currency: string; payment_date: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedInterestPaymentResponse:v1'; }",
    markdown:
      "## get_interest_payments\n\n`client.v2.accounts.getInterestPayments(account_id: string, end_date: string, start_date: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedInterestPaymentResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/interest_payments`\n\n\nGet interest payments made to the `Account` from yield-bearing cash holdings.\n\nCurrently, the only yield-bearing stablecoin accepted by Dinari is [USD+](https://usd.dinari.com/).\n\n\n### Parameters\n\n- `account_id: string`\n\n- `end_date: string`\n  End date, exclusive, in US Eastern time zone. ISO 8601 format, YYYY-MM-DD.\n\n- `start_date: string`\n  Start date, inclusive, in US Eastern time zone. ISO 8601 format, YYYY-MM-DD.\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { amount: number; currency: string; payment_date: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedInterestPaymentResponse:v1'; }`\n\n  - `data: { amount: number; currency: string; payment_date: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedInterestPaymentResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst response = await client.v2.accounts.getInterestPayments('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { end_date: '2019-12-27', start_date: '2019-12-27' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ data: { amount: number; currency: string; payment_date: string; stock_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedDividendPaymentResponse:v1'; }",
    markdown:
      "## get_dividend_payments\n\n`client.v2.accounts.getDividendPayments(account_id: string, end_date: string, start_date: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string, stock_id?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedDividendPaymentResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/dividend_payments`\n\nGet dividend payments made to the `Account` from dividend-bearing stock holdings.\n\n### Parameters\n\n- `account_id: string`\n\n- `end_date: string`\n  End date, exclusive, in US Eastern time zone. ISO 8601 format, YYYY-MM-DD.\n\n- `start_date: string`\n  Start date, inclusive, in US Eastern time zone. ISO 8601 format, YYYY-MM-DD.\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n- `stock_id?: string`\n  Optional ID of the `Stock` to filter by\n\n### Returns\n\n- `{ data: { amount: number; currency: string; payment_date: string; stock_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedDividendPaymentResponse:v1'; }`\n\n  - `data: { amount: number; currency: string; payment_date: string; stock_id: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedDividendPaymentResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst response = await client.v2.accounts.getDividendPayments('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { end_date: '2019-12-27', start_date: '2019-12-27' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    markdown:
      "## list\n\n`client.v2.accounts.orders.list(account_id: string, chain_id?: string, client_order_id?: string, limit?: number, next?: string, order?: 'asc' | 'desc', order_transaction_hash?: string, previous?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedAccountOrderResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/orders`\n\nGet a list of all `Orders` under the `Account`.<br>Optionally `Orders` can be filtered by chain ID, transaction hash, or client order ID.\n\n### Parameters\n\n- `account_id: string`\n\n- `chain_id?: string`\n  CAIP-2 formatted chain ID of the blockchain the `Order` was made on.\n\n- `client_order_id?: string`\n  Customer-supplied identifier to search for `Order`s.\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `order_transaction_hash?: string`\n  Transaction hash of the `Order`.\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; chain_id: string; created_dt: string; order_contract_address: string; order_side: 'BUY' | 'SELL'; order_tif: 'DAY' | 'GTC' | 'IOC' | 'FOK'; order_transaction_hash: string; order_type: 'MARKET' | 'LIMIT'; payment_token: string; status: string; stock_id: string; asset_token?: string; asset_token_quantity?: number; cancel_transaction_hash?: string; client_order_id?: string; fee?: number; limit_price?: number; order_request_id?: string; payment_token_quantity?: number; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountOrderResponse:v1'; }`\n\n  - `data: { id: string; chain_id: string; created_dt: string; order_contract_address: string; order_side: 'BUY' | 'SELL'; order_tif: 'DAY' | 'GTC' | 'IOC' | 'FOK'; order_transaction_hash: string; order_type: 'MARKET' | 'LIMIT'; payment_token: string; status: string; stock_id: string; asset_token?: string; asset_token_quantity?: number; cancel_transaction_hash?: string; client_order_id?: string; fee?: number; limit_price?: number; order_request_id?: string; payment_token_quantity?: number; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedAccountOrderResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst orders = await client.v2.accounts.orders.list('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(orders);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; asset_token_filled: number; asset_token_spent: number; chain_id: string; order_id: string; payment_token_filled: number; payment_token_spent: number; transaction_dt: string; transaction_hash: string; alloy_id?: string; payment_token_fee?: number; stock_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'; }",
    markdown:
      "## get_fulfillments\n\n`client.v2.accounts.orders.getFulfillments(account_id: string, order_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/orders/{order_id}/fulfillments`\n\nGet `OrderFulfillments` for a specific `Order`.\n\n### Parameters\n\n- `account_id: string`\n\n- `order_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; asset_token_filled: number; asset_token_spent: number; chain_id: string; order_id: string; payment_token_filled: number; payment_token_spent: number; transaction_dt: string; transaction_hash: string; alloy_id?: string; payment_token_fee?: number; stock_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'; }`\n\n  - `data: { id: string; asset_token_filled: number; asset_token_spent: number; chain_id: string; order_id: string; payment_token_filled: number; payment_token_spent: number; transaction_dt: string; transaction_hash: string; alloy_id?: string; payment_token_fee?: number; stock_id?: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst paginatedOrderFulfillment = await client.v2.accounts.orders.getFulfillments('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { account_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' });\n\nconsole.log(paginatedOrderFulfillment);\n```",
    perLanguage: {
      typescript: {
        method: 'client.v2.accounts.orders.getFulfillments',
        example:
          "import Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari({\n  apiKeyID: process.env['DINARI_API_KEY_ID'], // This is the default and can be omitted\n  apiSecretKey: process.env['DINARI_API_SECRET_KEY'], // This is the default and can be omitted\n});\n\nconst paginatedOrderFulfillment = await client.v2.accounts.orders.getFulfillments(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n  { account_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' },\n);\n\nconsole.log(paginatedOrderFulfillment.data);",
      },
      python: {
        method: 'v2.accounts.orders.get_fulfillments',
        example:
          'import os\nfrom dinari_api_sdk import Dinari\n\nclient = Dinari(\n    api_key_id=os.environ.get("DINARI_API_KEY_ID"),  # This is the default and can be omitted\n    api_secret_key=os.environ.get("DINARI_API_SECRET_KEY"),  # This is the default and can be omitted\n)\npaginated_order_fulfillment = client.v2.accounts.orders.get_fulfillments(\n    order_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n    account_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n)\nprint(paginated_order_fulfillment.data)',
      },
      java: {
        method: 'v2().accounts().orders().getFulfillments',
        example:
          'package com.dinari.api.example;\n\nimport com.dinari.api.client.DinariClient;\nimport com.dinari.api.client.okhttp.DinariOkHttpClient;\nimport com.dinari.api.models.v2.accounts.orderfulfillments.PaginatedOrderFulfillment;\nimport com.dinari.api.models.v2.accounts.orders.OrderGetFulfillmentsParams;\n\npublic final class Main {\n    private Main() {}\n\n    public static void main(String[] args) {\n        DinariClient client = DinariOkHttpClient.fromEnv();\n\n        OrderGetFulfillmentsParams params = OrderGetFulfillmentsParams.builder()\n            .accountId("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")\n            .orderId("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")\n            .build();\n        PaginatedOrderFulfillment paginatedOrderFulfillment = client.v2().accounts().orders().getFulfillments(params);\n    }\n}',
      },
      go: {
        method: 'client.V2.Accounts.Orders.GetFulfillments',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/dinaricrypto/dinari-api-sdk-go"\n\t"github.com/dinaricrypto/dinari-api-sdk-go/option"\n)\n\nfunc main() {\n\tclient := dinariapisdkgo.NewClient(\n\t\toption.WithAPIKeyID("My API Key ID"),\n\t\toption.WithAPISecretKey("My API Secret Key"),\n\t)\n\tpaginatedOrderFulfillment, err := client.V2.Accounts.Orders.GetFulfillments(\n\t\tcontext.TODO(),\n\t\t"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n\t\tdinariapisdkgo.V2AccountOrderGetFulfillmentsParams{\n\t\t\tAccountID: "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n\t\t},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", paginatedOrderFulfillment.Data)\n}\n',
      },
      php: {
        method: 'v2->accounts->orders->getFulfillments',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(\n  apiKeyID: 'My API Key ID',\n  apiSecretKey: 'My API Secret Key',\n  environment: 'sandbox',\n);\n\n$paginatedOrderFulfillment = $client->v2->accounts->orders->getFulfillments(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n  accountID: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n  limit: 20,\n  next: 'next',\n  order: 'asc',\n  previous: 'previous',\n);\n\nvar_dump($paginatedOrderFulfillment);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; asset_token_filled: number; asset_token_spent: number; chain_id: string; order_id: string; payment_token_filled: number; payment_token_spent: number; transaction_dt: string; transaction_hash: string; alloy_id?: string; payment_token_fee?: number; stock_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'; }",
    markdown:
      "## query\n\n`client.v2.accounts.orderFulfillments.query(account_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', order_ids?: string[], previous?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/order_fulfillments`\n\nQuery `OrderFulfillments` under the `Account`.\n\n### Parameters\n\n- `account_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `order_ids?: string[]`\n  List of `Order` IDs to query `AccountOrderFulfillment` for.\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; asset_token_filled: number; asset_token_spent: number; chain_id: string; order_id: string; payment_token_filled: number; payment_token_spent: number; transaction_dt: string; transaction_hash: string; alloy_id?: string; payment_token_fee?: number; stock_id?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'; }`\n\n  - `data: { id: string; asset_token_filled: number; asset_token_spent: number; chain_id: string; order_id: string; payment_token_filled: number; payment_token_spent: number; transaction_dt: string; transaction_hash: string; alloy_id?: string; payment_token_fee?: number; stock_id?: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedAccountOrderFulfillmentResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst paginatedOrderFulfillment = await client.v2.accounts.orderFulfillments.query('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(paginatedOrderFulfillment);\n```",
    perLanguage: {
      typescript: {
        method: 'client.v2.accounts.orderFulfillments.query',
        example:
          "import Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari({\n  apiKeyID: process.env['DINARI_API_KEY_ID'], // This is the default and can be omitted\n  apiSecretKey: process.env['DINARI_API_SECRET_KEY'], // This is the default and can be omitted\n});\n\nconst paginatedOrderFulfillment = await client.v2.accounts.orderFulfillments.query(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n);\n\nconsole.log(paginatedOrderFulfillment.data);",
      },
      python: {
        method: 'v2.accounts.order_fulfillments.query',
        example:
          'import os\nfrom dinari_api_sdk import Dinari\n\nclient = Dinari(\n    api_key_id=os.environ.get("DINARI_API_KEY_ID"),  # This is the default and can be omitted\n    api_secret_key=os.environ.get("DINARI_API_SECRET_KEY"),  # This is the default and can be omitted\n)\npaginated_order_fulfillment = client.v2.accounts.order_fulfillments.query(\n    account_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n)\nprint(paginated_order_fulfillment.data)',
      },
      java: {
        method: 'v2().accounts().orderFulfillments().query',
        example:
          'package com.dinari.api.example;\n\nimport com.dinari.api.client.DinariClient;\nimport com.dinari.api.client.okhttp.DinariOkHttpClient;\nimport com.dinari.api.models.v2.accounts.orderfulfillments.OrderFulfillmentQueryParams;\nimport com.dinari.api.models.v2.accounts.orderfulfillments.PaginatedOrderFulfillment;\n\npublic final class Main {\n    private Main() {}\n\n    public static void main(String[] args) {\n        DinariClient client = DinariOkHttpClient.fromEnv();\n\n        PaginatedOrderFulfillment paginatedOrderFulfillment = client.v2().accounts().orderFulfillments().query("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e");\n    }\n}',
      },
      go: {
        method: 'client.V2.Accounts.OrderFulfillments.Query',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/dinaricrypto/dinari-api-sdk-go"\n\t"github.com/dinaricrypto/dinari-api-sdk-go/option"\n)\n\nfunc main() {\n\tclient := dinariapisdkgo.NewClient(\n\t\toption.WithAPIKeyID("My API Key ID"),\n\t\toption.WithAPISecretKey("My API Secret Key"),\n\t)\n\tpaginatedOrderFulfillment, err := client.V2.Accounts.OrderFulfillments.Query(\n\t\tcontext.TODO(),\n\t\t"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n\t\tdinariapisdkgo.V2AccountOrderFulfillmentQueryParams{},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", paginatedOrderFulfillment.Data)\n}\n',
      },
      php: {
        method: 'v2->accounts->orderFulfillments->query',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(\n  apiKeyID: 'My API Key ID',\n  apiSecretKey: 'My API Secret Key',\n  environment: 'sandbox',\n);\n\n$paginatedOrderFulfillment = $client->v2->accounts->orderFulfillments->query(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n  limit: 20,\n  next: 'next',\n  order: 'asc',\n  orderIDs: ['182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e'],\n  previous: 'previous',\n);\n\nvar_dump($paginatedOrderFulfillment);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; account_id: string; created_dt: string; payment_token_amount: number; recipient_account_id: string; status: 'PENDING' | 'SUBMITTED' | 'ERROR' | 'CANCELLED'; updated_dt: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedWithdrawalRequestResponse:v1'; }",
    markdown:
      "## list\n\n`client.v2.accounts.withdrawalRequests.list(account_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: object[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedWithdrawalRequestResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/withdrawal_requests`\n\nList `WithdrawalRequests` under the `Account`, sorted by most recent.\n\n### Parameters\n\n- `account_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; account_id: string; created_dt: string; payment_token_amount: number; recipient_account_id: string; status: 'PENDING' | 'SUBMITTED' | 'ERROR' | 'CANCELLED'; updated_dt: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedWithdrawalRequestResponse:v1'; }`\n\n  - `data: { id: string; account_id: string; created_dt: string; payment_token_amount: number; recipient_account_id: string; status: 'PENDING' | 'SUBMITTED' | 'ERROR' | 'CANCELLED'; updated_dt: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedWithdrawalRequestResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst withdrawalRequests = await client.v2.accounts.withdrawalRequests.list('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(withdrawalRequests);\n```",
    perLanguage: {
      typescript: {
        method: 'client.v2.accounts.withdrawalRequests.list',
        example:
          "import Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari({\n  apiKeyID: process.env['DINARI_API_KEY_ID'], // This is the default and can be omitted\n  apiSecretKey: process.env['DINARI_API_SECRET_KEY'], // This is the default and can be omitted\n});\n\nconst withdrawalRequests = await client.v2.accounts.withdrawalRequests.list(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n);\n\nconsole.log(withdrawalRequests.data);",
      },
      python: {
        method: 'v2.accounts.withdrawal_requests.list',
        example:
          'import os\nfrom dinari_api_sdk import Dinari\n\nclient = Dinari(\n    api_key_id=os.environ.get("DINARI_API_KEY_ID"),  # This is the default and can be omitted\n    api_secret_key=os.environ.get("DINARI_API_SECRET_KEY"),  # This is the default and can be omitted\n)\nwithdrawal_requests = client.v2.accounts.withdrawal_requests.list(\n    account_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n)\nprint(withdrawal_requests.data)',
      },
      java: {
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; account_id: string; chain_id: string; payment_token_address: string; payment_token_amount: number; recipient_account_id: string; status: brokerage_order_status; transaction_dt: string; transaction_hash: string; withdrawal_request_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedWithdrawalResponse:v1'; }",
    markdown:
      "## list\n\n`client.v2.accounts.withdrawals.list(account_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string, withdrawal_request_id?: string): { data: withdrawal[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedWithdrawalResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/withdrawals`\n\nGet a list of all `Withdrawals` under the `Account`, sorted by most recent.\n\n### Parameters\n\n- `account_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n- `withdrawal_request_id?: string`\n  ID of the `WithdrawalRequest` to find `Withdrawals` for.\n\n### Returns\n\n- `{ data: { id: string; account_id: string; chain_id: string; payment_token_address: string; payment_token_amount: number; recipient_account_id: string; status: brokerage_order_status; transaction_dt: string; transaction_hash: string; withdrawal_request_id: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedWithdrawalResponse:v1'; }`\n\n  - `data: { id: string; account_id: string; chain_id: string; payment_token_address: string; payment_token_amount: number; recipient_account_id: string; status: string; transaction_dt: string; transaction_hash: string; withdrawal_request_id: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedWithdrawalResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst withdrawals = await client.v2.accounts.withdrawals.list('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(withdrawals);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    markdown:
      "## list\n\n`client.v2.accounts.tokenTransfers.list(account_id: string, limit?: number, next?: string, order?: 'asc' | 'desc', previous?: string): { data: token_transfer[]; pagination_metadata: pagination_metadata; _sv?: 'PaginatedTokenTransferResponse:v1'; }`\n\n**get** `/api/v2/accounts/{account_id}/token_transfers`\n\n\nGet `TokenTransfer`s made from this `Account`.\n\nA `TokenTransfer` represents a transfer of tokens through the Dinari platform from one `Account` to another.\nAs such, only `Account`s that are connected to Dinari-managed `Wallet`s can initiate `TokenTransfer`s.\n\n\n### Parameters\n\n- `account_id: string`\n\n- `limit?: number`\n  Number of results to return\n\n- `next?: string`\n  Cursor for next page\n\n- `order?: 'asc' | 'desc'`\n  Sort order\n\n- `previous?: string`\n  Cursor for previous page\n\n### Returns\n\n- `{ data: { id: string; chain_id: string; created_dt: string; quantity: number; recipient_account_id: string; sender_account_id: string; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE' | 'FAILED'; token_address: string; updated_dt: string; transaction_hash?: string; }[]; pagination_metadata: { next?: string; previous?: string; }; _sv?: 'PaginatedTokenTransferResponse:v1'; }`\n\n  - `data: { id: string; chain_id: string; created_dt: string; quantity: number; recipient_account_id: string; sender_account_id: string; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETE' | 'FAILED'; token_address: string; updated_dt: string; transaction_hash?: string; }[]`\n  - `pagination_metadata: { next?: string; previous?: string; }`\n  - `_sv?: 'PaginatedTokenTransferResponse:v1'`\n\n### Example\n\n```typescript\nimport Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari();\n\nconst tokenTransfers = await client.v2.accounts.tokenTransfers.list('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');\n\nconsole.log(tokenTransfers);\n```",
    perLanguage: {
      typescript: {
        method: 'client.v2.accounts.tokenTransfers.list',
        example:
          "import Dinari from '@dinari/api-sdk';\n\nconst client = new Dinari({\n  apiKeyID: process.env['DINARI_API_KEY_ID'], // This is the default and can be omitted\n  apiSecretKey: process.env['DINARI_API_SECRET_KEY'], // This is the default and can be omitted\n});\n\nconst tokenTransfers = await client.v2.accounts.tokenTransfers.list(\n  '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',\n);\n\nconsole.log(tokenTransfers.data);",
      },
      python: {
        method: 'v2.accounts.token_transfers.list',
        example:
          'import os\nfrom dinari_api_sdk import Dinari\n\nclient = Dinari(\n    api_key_id=os.environ.get("DINARI_API_KEY_ID"),  # This is the default and can be omitted\n    api_secret_key=os.environ.get("DINARI_API_SECRET_KEY"),  # This is the default and can be omitted\n)\ntoken_transfers = client.v2.accounts.token_transfers.list(\n    account_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",\n)\nprint(token_transfers.data)',
      },
      java: {
